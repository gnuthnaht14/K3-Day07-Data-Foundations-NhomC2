...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        # Base case 1: text is already within chunk_size
        if len(current_text) <= self.chunk_size:
            return [current_text]

        # Base case 2: no remaining separators or separator is empty string
        ...
        # If current separator is not present in text, try next separators
        if sep not in current_text:
            return self._split(current_text, next_seps)

        # Split text by separator and merge adjacent parts up to chunk_size
