...
def resolve_embedder() -> tuple[Callable[[str], list[float]], str]:
    """Pick embedder backend based on EMBEDDING_PROVIDER env var."""
    provider = os.environ.get(EMBEDDING_PROVIDER_ENV, "mock").strip().lower()

    if provider == "local":
        embedder = LocalEmbedder(LOCAL_EMBEDDING_MODEL)
        return embedder, embedder._backend_name
    if provider == "openai":
        embedder = OpenAIEmbedder(OPENAI_EMBEDDING_MODEL)
        return embedder, embedder._backend_name
...
def main() -> int:
    embed_fn, backend_name = resolve_embedder()

    store = build_knowledge_base(DATA_DIR, embed_fn, chunker=chunker)
